...
function registerGlobalHandler() {
  ...
  function onGlobalError(ev) {
    if (ev && !ev.error && ev.message == "Script error.") {
      var scriptErrorReport = createReport();
      scriptErrorReport.setError(new Error("Script error."));
      scriptErrorReport.addObjectAttributes({
        referrer: window && window.location && window.location.href
      });
      scriptErrorReport.send();
    } else if (ev.error) {
      report(ev.error);
    } else {
